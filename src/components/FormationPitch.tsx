...
export function FormationPitch({ formation, placed, highlightOpen, className }: FormationPitchProps) {
  ...
  return (
    ...
    >
      ...
      {slots.map((slot) => {
        const placedPlayer = placedBySlot.get(slot.id);
        return (
          <SlotNode
            key={slot.id}
            slot={slot}
            placed={placedPlayer}
            highlightOpen={highlightOpen && !placedPlayer}
          />
        );
      })}
    </div>
  ...
  return (
    ...
    >
      {placed ? (
        <>
          <div
            className={`flex h-9 w-9 items-center justify-center rounded-full border-2 text-sm font-display shadow-lg ${
        ...
        <>
          <div
            className={`flex h-9 w-9 items-center justify-center rounded-full border-2 border-dashed text-[10px] font-bold ${
              highlightOpen ? 'animate-pulse border-gold-400 bg-gold-400/10 text-gold-300' : 'border-white/30 text-white/40'
            }`}
          >
            {slot.label}
          </div>
          <span className="mt-0.5 text-[9px] uppercase tracking-wide text-white/35">vazio</span>
        </>
      )}
    </div>
  );
